...
HEX_KEY=${3:-0123456789abcdef0123456789abcdef0123456789abcdef0123456789}
SSRC=${4:-3735928559}
...
if ! command -v gst-launch-1.0 &> /dev/null; then
  ...
  echo "Install with: sudo apt install gstreamer1.0-tools gstreamer1.0-plugins-bad (Ubuntu/Debian) or brew install gstreamer gst-plugins-bad (macOS)"
  exit 1
fi

if ! gst-inspect-1.0 srtpenc &> /dev/null; then
  echo "Error: GStreamer element 'srtpenc' not found (ships in gst-plugins-bad)"
  exit 1
fi
...
gst-launch-1.0 -e \
  ...
  ! srtpenc key="(buffer)${HEX_KEY}" rtp-cipher=aes-128-icm rtp-auth=hmac-sha1-80 rtcp-cipher=aes-128-icm rtcp-auth=hmac-sha1-80 \

...
echo "export DYNAMODB_TABLE_NAME=\"$(echo "$OUTPUTS" | jq -r '.[] | select(.OutputKey=="DynamoDBTableName") | .OutputValue')\""
echo "export CLOUDFRONT_DOMAIN_NAME=\"$(echo "$OUTPUTS" | jq -r '.[] | select(.OutputKey=="CloudFrontURL") | .OutputValue')\""
echo "export BUCKET_NAME=\"$(echo "$OUTPUTS" | jq -r '.[] | select(.OutputKey=="VideoBucketName") | .OutputValue')\""

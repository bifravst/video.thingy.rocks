...
void describe('stream-testsrc-to-srtp.py', () => {
	...
	void describe(
		...
		() => {
			/**
			 * Finds the first of the real ingest ports 6000-6009 that is free.
			 *
			 * The sender only accepts the real ingest ports, and in v3 the helper
			 * binds the ingest port itself - the sender sends to it directly, the
			 * way a device does in production. No relay sits in between any more,
			 * which is the point of the redesign.
			 */
			const freeIngestPort = async (): Promise<number> => {
				for (let port = 6000; port <= 6009; port++) {
					const probe = dgram.createSocket('udp6')
					const bound = await new Promise<boolean>((resolve) => {
						probe.once('error', () => resolve(false))
						probe.bind(port, () => resolve(true))
					})
					probe.close()
					if (bound) return port
				}
				throw new Error('no port in 6000-6009 is free')
			}
			...
			const streamWith = async (
				...
				const key = freshKey()
				const port = await freeIngestPort()
				...
				const sender = spawn(
					...
					[
						SENDER,
						// IPv4 loopback: the helper's socket is dual-stack like the
						// service's listener, and this reaches it through the mapped
						// address on every machine that can run the suite.
						'127.0.0.1',
						String(port),
						'--ssrc',
						String(SSRC),
